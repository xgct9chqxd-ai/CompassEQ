...
namespace
{
    ...
    static juce::Image createMatteNoiseTexture(int size = 512)
    {
        ...
        for (int y = 0; y < size; ++y)
        {
            for (int x = 0; x < size; ++x)
            {
                ...
                {
                    float nx = amp * (rnd.nextFloat() * 2.0f - 1.0f) * std::sin((float)x * freqX + (float)y * freqY * 0.2f);
                    float ny = amp * (rnd.nextFloat() * 0.5f - 0.25f) * std::cos((float)y * freqY + (float)x * freqX * 0.1f);
    ...
    static void drawFaceplateStage3ZonedNoSeams(
        ...
        const juce::Image &cosmicHazeImage,
    ...
    (void) colHF;
        ...
        // Enhanced background: deeper matte metal + subtle brushed texture + soft vignette
        {
            const auto b = editor.toFloat();
            // A) Deeper matte metal gradient with slight center brighten (pressed panel feel)
            juce::ColourGradient baseGrad(
                juce::Colour(0xFF333333), b.getCentreX(), b.getY() + b.getHeight() * 0.1f,
                juce::Colour(0xFF222222), b.getCentreX(), b.getBottom() - b.getHeight() * 0.1f,
                false);
            baseGrad.addColour(0.5, juce::Colour(0xFF2A2A2A)); // subtle center lift
            g.setGradientFill(baseGrad);
            g.fillAll();
            // B) Cosmic haze overlay (subtle)
            ...
            // B) Existing minimal grain overlay (keep it – perfect as-is)
            ...
            // C) Ultra-subtle horizontal brushed metal texture (your excellent matteNoise)
            if (matteNoise.isValid())
            {
                juce::Graphics::ScopedSaveState ss(g);
                g.setTiledImageFill(matteNoise, 0, 0, 0.06f); // very light – adjust 0.04–0.08 to taste
                g.fillRect(editor);
            }
            // D) Softer, wider vignette for gentle framing
            {
                ...
                juce::ColourGradient vignette(juce::Colours::transparentBlack, cx, cy,
                                              juce::Colours::black.withAlpha(0.08f), cx, cy + radius,
                                              true);
                g.setGradientFill(vignette);
                g.fillRect(b);
            }
            // E) Optional: very subtle outer bevel for "hardware chassis" depth
            // Comment out or delete if you don't want it
            {
                const auto bounds = editor.toFloat().reduced(1.0f);
                // Top/left highlight
        ...
        // - position = computed divider x's between equal-width lanes only
        {
            ...
            if (!bandsRect.isEmpty())
            {
                ...
                constexpr float kLaneTopExtra = 30.0f;
                ...
                const float y2 = (float)bandsRect.getBottom() + 18.0f;
                ...
                drawBandPanel(laneLF, juce::Colour(0xFF0088FF).withAlpha(0.28f), juce::Colour(0xFF003366).withAlpha(0.20f));
                drawBandPanel(laneLMF, juce::Colour(0xFFAA33FF).withAlpha(0.26f), juce::Colour(0xFF4A146E).withAlpha(0.20f));
                drawBandPanel(laneHMF, juce::Colour(0xFF88FF88).withAlpha(0.25f), juce::Colour(0xFF2B6B2B).withAlpha(0.18f));
                drawBandPanel(laneHF, juce::Colour(0xFFFF4444).withAlpha(0.28f), juce::Colour(0xFF7A1E1E).withAlpha(0.20f));
                ...
                auto glowAt = [&](juce::Rectangle<int> knob, juce::Colour c)
                {
                    ...
                    juce::ColourGradient glow(c.withAlpha(0.10f), centre.x, centre.y,
                                              ...
                                              true /* radial */);
                    glow.addColour(0.0, c.withAlpha(0.06f));
                    glow.addColour(0.25, c.withAlpha(0.03f));
    ...
    juce::Slider &s)
{
    ...
    else if (nm.containsIgnoreCase("Input Trim"))
    {
        // Premium neutral with faint LF (blue) bias (keep subtle)
        auto c = capColourForHue(UIStyle::Colors::bandHueLF);
        bandColour = c.withMultipliedSaturation(0.38f).darker(0.22f);
    }
    else if (nm.containsIgnoreCase("Output Trim"))
    {
        // Premium neutral with faint HF (warm/red) bias (keep subtle)
        auto c = capColourForHue(UIStyle::Colors::bandHueHF);
        c = c.withRotatedHue(-0.010f);
        bandColour = c.withMultipliedSaturation(0.38f).darker(0.22f);
    }
    ...
    const bool isBandPrimaryKnob =
        isBandKnob && (nm.containsIgnoreCase("frequency") || nm.containsIgnoreCase("gain") || nm.containsIgnoreCase("q"));
    ...
    // Keep existing hover glow above (Pass 4). This block replaces the SSL-metal knob body.
    {
        ...
        // Central grab notch (rotates with value)
        {
            ...
            // Gain dominance: larger bright white central notch + strong rim
            float gainDotR = dotR;
            juce::Colour gainFill = juce::Colour(0xFFD6D6D6);
            float gainFillAlpha = 0.85f;
            float gainRimAlpha = 0.10f;
            if (nm.containsIgnoreCase("Gain"))
            {
                gainDotR *= 1.28f;             // 28% larger — strong dominance
                gainFill = juce::Colours::white;
                gainFillAlpha = 1.0f;
                gainRimAlpha = 0.38f;          // very strong rim
            }
            juce::Rectangle<float> gainDot(nx - gainDotR, ny - gainDotR, gainDotR * 2.0f, gainDotR * 2.0f);

            g.fillEllipse(gainDot.translated(0.0f, 0.9f));
            g.setColour(juce::Colours::black.withAlpha(0.30f));
            g.setColour(gainFill.withAlpha(gainFillAlpha));
            g.fillEllipse(gainDot);
            g.fillEllipse(dot);
            g.setColour(juce::Colours::white.withAlpha(gainRimAlpha));
            g.drawEllipse(gainDot, 1.0f);
        }
    }
...
CompassEQAudioProcessorEditor::CompassEQAudioProcessorEditor(CompassEQAudioProcessor &p)
    ...
    lfFreq.setName("LF Frequency");
    lfGain.setName("LF Gain");
    lmfFreq.setName("LMF Frequency");
    lmfGain.setName("LMF Gain");
    lmfQ.setName("LMF Q");
    hmfFreq.setName("HMF Frequency");
    hmfGain.setName("HMF Gain");
    hmfQ.setName("HMF Q");
    hfFreq.setName("HF Frequency");
    hfGain.setName("HF Gain");
    ...
    auto addKnob = [this](juce::Slider &s)
    { addAndMakeVisible(s); };
    // Add sliders
    addKnob(lfFreq);
    addKnob(lfGain);
...
{
    if (backgroundGrainTexture.isValid())
        return;

    // Load your real embedded image once
    backgroundGrainTexture = juce::ImageCache::getFromMemory(
        BinaryData::grain_texture_png,
        BinaryData::grain_texture_pngSize
    );

    // Optional: fallback if load fails (should never happen)
    if (!backgroundGrainTexture.isValid())
    {
        // Keep old procedural as backup, or just leave empty
        backgroundGrainTexture = juce::Image(juce::Image::RGB, 256, 256, false);
        backgroundGrainTexture.clear(backgroundGrainTexture.getBounds());
    }
}

void CompassEQAudioProcessorEditor::ensureCosmicHaze()
{
    if (cosmicHazeTexture.isValid())
        return;

    cosmicHazeTexture = juce::ImageCache::getFromMemory(
        BinaryData::cosmic_haze_png,
        BinaryData::cosmic_haze_pngSize
    );

    if (! cosmicHazeTexture.isValid())
    {
        cosmicHazeTexture = juce::Image(juce::Image::RGB, 256, 256, false);
        cosmicHazeTexture.clear(cosmicHazeTexture.getBounds());
    }
}
void CompassEQAudioProcessorEditor::renderStaticLayer(juce::Graphics &g, float scaleKey, float physicalScale)
{
    ...
    auto drawLegendBelow = [&g, &microFont, kLegendA, physicalScale, drawEngravedFitted](const char *txt, juce::Rectangle<int> b, int yOffset)
    {
        ...
        g.setFont(microFont.withHeight(10.0f).withExtraKerningFactor(0.02f));
        // Phase 2: Snap baseline Y
    ...
    // Bottom trim labels: move IN/OUT below knobs (centered), and remove any "TRIM" wording.
    {
        ...
        g.setFont(f);
        drawEngravedFitted("INPUT", inLabel.getX(), inLabel.getY(), inLabel.getWidth(), inLabel.getHeight(),
                           juce::Justification::centred, 1, kSectionA, juce::Colours::white);
        drawEngravedFitted("OUTPUT", outLabel.getX(), outLabel.getY(), outLabel.getWidth(), outLabel.getHeight(),
                           juce::Justification::centred, 1, kSectionA, juce::Colours::white);
    }
    // Legends
    drawLegendBelow("kHz", lfFreq.getBounds(), 2);
    drawLegendBelow("GB", lfGain.getBounds(), 2);
    drawLegendBelow("kHz", lmfFreq.getBounds(), 2);
    drawLegendBelow("GB", lmfGain.getBounds(), 2);
    drawLegendBelow("Q", lmfQ.getBounds(), 2);
    drawLegendBelow("kHz", hmfFreq.getBounds(), 2);
    drawLegendBelow("GB", hmfGain.getBounds(), 2);
    drawLegendBelow("Q", hmfQ.getBounds(), 2);
    drawLegendBelow("kHz", hfFreq.getBounds(), 2);
    drawLegendBelow("GB", hfGain.getBounds(), 2);
    ...
    // Vector-only, no assets, no layout changes. Uses engraved text helper for numbers.
    {
                                                                                                                 ...
                                                                                                                 int numberCount, bool isFilter = false)
        {
            ...
            g.setFont(microFont.withHeight(isFilter ? 14.5f : 12.5f).withExtraKerningFactor(isFilter ? -0.015f : -0.03f));
            const float px1 = 1.0f / juce::jmax(1.0f, physicalScale);
            for (int i = 0; i < numberCount; ++i)
            {
                const float t = (float)i / (float)(numberCount - 1);
                const float ang = startRad + t * range;
                float rText = radius + 10.0f;

                // Special-case: LF freq endpoints "20" and "800" — push outward slightly
                const bool isLF20to800 =
                    (numberCount == 5 &&
                     numbers != nullptr &&
                     numbers[0] != nullptr &&
                     numbers[numberCount - 1] != nullptr &&
                     numbers[0][0] == '2' && numbers[0][1] == '0' && numbers[0][2] == '\0' &&
                     numbers[numberCount - 1][0] == '8' && numbers[numberCount - 1][1] == '0' &&
                     numbers[numberCount - 1][2] == '0' && numbers[numberCount - 1][3] == '\0');

                if (isLF20to800 && (i == 0 || i == numberCount - 1))
                    rText += 6.0f;


                // Special-case: LMF freq endpoints "120" and "4k" — push outward slightly (no vertical nudge)
                const bool isLMF120to4k =
                    (numberCount == 5 &&
                     numbers != nullptr &&
                     numbers[0] != nullptr &&
                     numbers[numberCount - 1] != nullptr &&
                     numbers[0][0] == '1' && numbers[0][1] == '2' && numbers[0][2] == '0' && numbers[0][3] == 0 &&
                     numbers[numberCount - 1][0] == '4' && numbers[numberCount - 1][1] == 'k' && numbers[numberCount - 1][2] == 0);

                if (isLMF120to4k && (i == 0 || i == numberCount - 1))
                    rText += 4.0f;

                // Special-case: HMF freq endpoints "600" and "15k" — identify set for tiny y adjustment
                const bool isHMF600to15k =
                    (numberCount == 5 &&
                     numbers != nullptr &&
                     numbers[0] != nullptr &&
                     numbers[numberCount - 1] != nullptr &&
                     numbers[0][0] == '6' && numbers[0][1] == '0' && numbers[0][2] == '0' && numbers[0][3] == 0 &&
                     numbers[numberCount - 1][0] == '1' && numbers[numberCount - 1][1] == '5' && numbers[numberCount - 1][2] == 'k' &&
                     numbers[numberCount - 1][3] == 0);


                // Special-case: HF freq endpoints "1.5k" and "22k" — push outward slightly (no vertical nudge)
                const bool isHF1p5to22k =
                    (numberCount == 5 &&
                     numbers != nullptr &&
                     numbers[0] != nullptr &&
                     numbers[numberCount - 1] != nullptr &&
                     // first = "1.5k"
                     numbers[0][0] == '1' && numbers[0][1] == '.' && numbers[0][2] == '5' && numbers[0][3] == 'k' && numbers[0][4] == 0 &&
                     // last = "22k"
                     numbers[numberCount - 1][0] == '2' && numbers[numberCount - 1][1] == '2' && numbers[numberCount - 1][2] == 'k' && numbers[numberCount - 1][3] == 0);

                if (isHF1p5to22k && (i == 0 || i == numberCount - 1))
                    rText += 4.0f;

                float yNudge = 0.0f;

                // LF freq endpoints "20" and "800" were pushed outward; nudge them slightly UP (visual balance)
                if (isLF20to800 && (i == 0 || i == numberCount - 1))
                    yNudge = -5.0f;

                // LMF freq endpoints "120" and "4k" — nudge slightly UP (after outward push)
                if (isLMF120to4k && (i == 0 || i == numberCount - 1))
                    yNudge = -3.0f;

                // HMF endpoints "600" and "15k" — nudge slightly DOWN (visual alignment)
                if (isHMF600to15k && (i == 0 || i == numberCount - 1))
                    yNudge = 1.0f;

                // HF endpoints "1.5k" and "22k" — nudge slightly UP (after outward push)
                if (isHF1p5to22k && (i == 0 || i == numberCount - 1))
                    yNudge = -3.0f;

                float xNudge = 0.0f;
                
                // LMF freq label "4k" — nudge slightly LEFT (fine alignment)
                if (isLMF120to4k && i == numberCount - 1)
                    xNudge = -1.5f;

                
                // LF freq label "20" — nudge slightly RIGHT (half-step fine alignment)
                if (isLF20to800 && i == 0)
                    xNudge = +1.5f;


// HMF freq label "600" — nudge slightly LEFT (fine alignment only)
const bool isHMF600 =
    (numberCount == 5 &&
     numbers != nullptr &&
     numbers[0] != nullptr &&
...
if (isHMF600 && i == 0)
    xNudge = -2.0f;
                ...
                const float x = cx + rText * std::cos(ang) + xNudge;
                const float y = cy + rText * std::sin(ang) + yNudge;
                const int w = 24;
                const int h = 14;
                ...
                if (isFilter)
                {
                    ...
                    g.drawFittedText(numbers[i], xi - 1, yi,     w, h, juce::Justification::centred, 1);
                    g.drawFittedText(numbers[i], xi + 1, yi,     w, h, juce::Justification::centred, 1);
                    g.drawFittedText(numbers[i], xi,     yi - 1, w, h, juce::Justification::centred, 1);
                    g.drawFittedText(numbers[i], xi,     yi + 1, w, h, juce::Justification::centred, 1);
                }
    ...
    drawTick(lfGain.getBounds(), -2);
    drawTick(lmfFreq.getBounds(), -2);
    drawTick(lmfGain.getBounds(), -2);
    drawTick(lmfQ.getBounds(), -2);
    drawTick(hmfFreq.getBounds(), -2);
    drawTick(hmfGain.getBounds(), -2);
    drawTick(hmfQ.getBounds(), -2);
    drawTick(hfFreq.getBounds(), -2);
    drawTick(hfGain.getBounds(), -2);
    ...
    if constexpr (kPaintAuditOverlay == 1)
    {
        ...
        box(lfFreq.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(lfGain.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(lmfFreq.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(lmfGain.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(lmfQ.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(hmfFreq.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(hmfGain.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(hmfQ.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(hfFreq.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
        box(hfGain.getBounds(), UIStyle::UIAlpha::auditOverlayKnob);
...
void CompassEQAudioProcessorEditor::paint(juce::Graphics &g)
{
    ...
    const bool cacheValid = staticCache.valid() && std::abs(staticCache.scaleKey - sk) < 0.001f && staticCache.pixelW == pw && staticCache.pixelH == ph;
...
void CompassEQAudioProcessorEditor::handleAsyncUpdate()
{
    ...
    if (!staticCacheDirty.load(std::memory_order_acquire) && std::abs(staticCache.scaleKey - sk) < 0.001f && staticCache.valid() && staticCache.pixelW == pw && staticCache.pixelH == ph)
        return;
...
void CompassEQAudioProcessorEditor::resized()
{
    ...
    // ===== Phase 4.0 — Asset Slot Map (derived from existing bounds) =====
    {
        ...
        assetSlots.lfFreq = lfFreq.getBounds();
        assetSlots.lfGain = lfGain.getBounds();
        assetSlots.lmfFreq = lmfFreq.getBounds();
        assetSlots.lmfGain = lmfGain.getBounds();
        assetSlots.lmfQ = lmfQ.getBounds();
        assetSlots.hmfFreq = hmfFreq.getBounds();
        assetSlots.hmfGain = hmfGain.getBounds();
        assetSlots.hmfQ = hmfQ.getBounds();
        assetSlots.hfFreq = hfFreq.getBounds();
        assetSlots.hfGain = hfGain.getBounds();
        ...
        assetSlots.bandsUnion =
            assetSlots.lfFreq.getUnion(assetSlots.lfGain)
                .getUnion(assetSlots.lmfFreq)
                .getUnion(assetSlots.lmfGain)
                .getUnion(assetSlots.lmfQ)
                .getUnion(assetSlots.hmfFreq)
                .getUnion(assetSlots.hmfGain)
                .getUnion(assetSlots.hmfQ)
                .getUnion(assetSlots.hfFreq)
                .getUnion(assetSlots.hfGain);
